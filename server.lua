...
ESX.RegisterServerCallback("wcukBilling:GetInvoices", function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)

	MySQL.Async.fetchAll('SELECT * FROM wcukBilling WHERE receiver_identifier = @identifier ORDER BY CASE WHEN status = "unpaid" THEN 1 WHEN status = "autopaid" THEN 2 WHEN status = "paid" THEN 3 WHEN status = "cancelled" THEN 4 END ASC, id DESC', {
...
RegisterServerEvent("wcukBilling:PayInvoice")
AddEventHandler("wcukBilling:PayInvoice", function(invoice_id)
	local xPlayer = ESX.GetPlayerFromId(source)

	MySQL.Async.fetchAll('SELECT * FROM wcukBilling WHERE id = @id', {
		['@id'] = invoice_id
	}, function(result)
		...
		if playerMoney < invoices.invoice_value then
			TriggerClientEvent('wcukNotify:Alert', xPlayer.source, "BILLING", "You don't have enough money!", 10000, 'error')
		else
			...
			MySQL.Async.execute('UPDATE wcukBilling SET status = @status, paid_date = CURRENT_TIMESTAMP WHERE id = @id', {
				['@status'] = 'paid',
				['@id'] = invoice_id
			})

			TriggerClientEvent('wcukNotify:Alert', xPlayer.source, "BILLING", "Invoice successfully paid!", 10000, 'success')
...
RegisterServerEvent("wcukBilling:CancelInvoice")
AddEventHandler("wcukBilling:CancelInvoice", function(invoice_id)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.Async.fetchAll('SELECT * FROM wcukBilling WHERE id = @id', {
		...
		MySQL.Async.execute('UPDATE wcukBilling SET status = "cancelled", paid_date = CURRENT_TIMESTAMP WHERE id = @id', {
			['@id'] = invoice_id
		})
		TriggerClientEvent('wcukNotify:Alert', xPlayer.source, "BILLING", "You have cancelled the invoice!", 10000, 'info')
		if Webhook ~= '' then
...
RegisterServerEvent("wcukBilling:CreateInvoice")
AddEventHandler("wcukBilling:CreateInvoice", function(data)
	local _source = ESX.GetPlayerFromId(source)
	local target = ESX.GetPlayerFromId(data.target)
	local webhookData = {}

	MySQL.Async.fetchAll('SELECT id FROM wcukBilling WHERE id = (SELECT MAX(id) FROM wcukBilling)', {}, function(result)
		webhookData = {
			id = result[1].id + 1,
	...
	if Config.LimitDate then
		MySQL.Async.insert('INSERT INTO wcukBilling (receiver_identifier, receiver_name, author_identifier, author_name, society, society_name, item, invoice_value, status, notes, sent_date, limit_pay_date) VALUES (@receiver_identifier, @receiver_name, @author_identifier, @author_name, @society, @society_name, @item, @invoice_value, @status, @notes, CURRENT_TIMESTAMP(), DATE_ADD(CURRENT_TIMESTAMP(), INTERVAL @limit_pay_date DAY))', {
			...
			TriggerClientEvent('wcukNotify:Alert', target.source, "BILLING", "You have just received a new invoice!", 10000, 'info')
			if Webhook ~= '' then
...
ESX.RegisterServerCallback("wcukBilling:GetSocietyInvoices", function(source, cb, society)
	local xPlayer = ESX.GetPlayerFromId(source)

	MySQL.Async.fetchAll('SELECT * FROM wcukBilling WHERE society_name = @society ORDER BY id DESC', {
		['@society'] = society
	}, function(result)
		local invoices = {}
...
function checkTimeLeft()
	MySQL.Async.fetchAll('SELECT *, TIMESTAMPDIFF(HOUR, limit_pay_date, CURRENT_TIMESTAMP()) AS "timeLeft" FROM wcukBilling WHERE status = @status', {
		['@status'] = 'unpaid'
	}, function(result)
				...
				for k, vl in pairs(whenToAddFees) do
					if v.fees_amount == k - 1 then
						if v.timeLeft >= vl*(-1) then
							MySQL.Async.execute('UPDATE wcukBilling SET fees_amount = @fees_amount, invoice_value = @invoice_value WHERE id = @id', {
								['@fees_amount'] = k,
						...
						MySQL.Async.execute('UPDATE users SET accounts = @playerAccount WHERE identifier = @target', {
							...
							TriggerEvent("esx_addonaccount:getSharedAccount", v.society, function(account2)
								if account2 ~= nil then
									...
									MySQL.Async.execute('UPDATE wcukBilling SET status = @paid, paid_date = CURRENT_TIMESTAMP() WHERE id = @id', {
										['@paid'] = 'autopaid',
										['@id'] = v.id
									})
								end
